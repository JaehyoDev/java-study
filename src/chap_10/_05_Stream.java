...
public class _05_Stream {
    public static void main(String[] args) {
        // 스트림 : 자바8에서 추가된 스트림은 람다를 활용할 수 있는 기술 중 하나
        // 데이터의 흐름으로 배열 또는 컬렉션 인스턴스에 함수 여러 개를 조합해서 필터링하고 가공 가능
        // 또한 람다를 이용해서 코드의 양을 줄이고 간결하게 표현 가능
        // 즉, 배열과 컬렉션을 함수형으로 처리 가능
        // 자바8 전에는 배열 또는 컬렉션 인스턴스를 다루는 방법은
        // for 또는 foreach 문을 돌면서 요소 하나씩을 꺼내서 다루는 방법이었다.

        // 스트림 생성
        // 생성 방법 1 : Arrays.stream()
        ...
        // 생성 방법 2 : Collection.stream()
        List<String> langList = new ArrayList<>();
        // langList.add("python");
        // langList.add("java");
        // 위처럼 반복없이 밑에 코드처럼 한 번에 가능하다.
        ...
        // 예제에서는 스트림을 계속 사용해야해서 매번 새롭게 만듬.
        // 스트림을 사용한다고 해서 원본 데이터가 훼손되지는 않음.

        // 90 점 이상인 점수만 출력
        Arrays.stream(scores).filter(x -> x >= 90).forEach(x -> System.out.println(x));
        // Arrays.stream(scores).filter(x -> x >= 90).forEach(System.out::println); // 위와 같음

        System.out.println("--------------------");

        // 90 점 이상인 사람의 수
        int count = (int) Arrays.stream(scores).filter(x -> x >= 90).count(); // long 또는 int 형변환
        ...
        // 3글자 이하의 언어 중에서 c 라는 글자를 포함하는 언어가 모두 있는 지 여부
        ...
        // c 라는 글자를 포함하는 언어를 대문자로 변경하여 저장
